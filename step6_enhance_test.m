...
param.file_name = 'data_2/中证500策略.xlsx';
...
% -------------------------------------------------------------------------
% 获取股票基本信息、日频信息、月频信息
load('data_2/stock_data','basic_info','daily_info','monthly_info');

% 获取因子暴露以及协方差估计结果
load('result_2/factorExpo.mat');
factorCov = importdata('result_2/factorCovEigenAdj.mat');
specialCov = importdata('result_2/specialCovBiasAdj.mat');

% 获取基准基准指数信息
load('data_2/index_data.mat')
indexInfo = getfield(index_data,param.targetIndex);
indexClose = getfield(indexInfo.close,param.baseType)'; 

% 获取各行业指数每日收盘价
load('data_2/indus_data.mat');
indusClose = indus_data.indus_close;
clear indus_data

% 获取个股收益率预测(如果指数内选股则获取指数内预测结果，否则全市场预测)
if param.selectInIndex
    forecastReturn = importdata(sprintf('data_2/%s.mat',param.targetIndex));
else
    forecastReturn = importdata('data_2/market.mat');
end
...
legend({'index\_inside\_enhancement','index','enhancement\_excess\_than\_index'},'location','northwest');
...
set(gcf,'color','w'); 
set(gcf,'position',[350,150,600,400])
legend({'index\_inside\_enhancement','index','enhancement\_excess\_than\_neutral'},'location','northwest');
set(gca,'FontSize',10,'FontWeight','bold','FontName','STFangSong','box','off');
xlim([1 length(dateStr)]);
...
set(gcf,'color','w'); 
set(gcf,'position',[350,150,600,400])
legend({'index\_inside\_neutral','index','neutral\_excess\_than\_index'},'location','northwest');
set(gca,'FontSize',10,'FontWeight','bold','FontName','STFangSong','box','off');
xlim([1 length(dateStr)]);
